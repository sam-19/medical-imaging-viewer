...
class GenericStudyLoader implements StudyLoader {
     ...
     * @return a promise containing the loaded studies as { title: string, studies: StudyObject[] }
     */
    public async loadFromFileSystem (fileTree: FileSystemItem, config: any = {}): Promise<{title: string, studies: StudyObject[]}[]> {
        if (!fileTree) {
            return []
        }
        const visits = []
